  ...
  render(){
    ...
    for(let i in this.state.locations){
      ...
      for(let j in this.state.locations[i].spaces){
        ...
        let element =(<ListGroupItem tag="button" action className="justify-content-between">{this.state.locations[i].spaces[j].Type} <Badge pill>{this.state.locations[i].spaces[j].Price}</Badge></ListGroupItem>);
        
        spaces.push(element);

      }
      let element = (
        <ListGroupItem style={{ background:"rgba(0,0,0,.125)" }} className={"justify-content-between"}>
          <div style={{display:"flex",marginBottom:"0.5rem"}}><h5 style={{flexGrow:1}}>{this.state.locations[i].Name}</h5>  <Button size="sm" color="primary">edit</Button>{' '}</div> 
          <ListGroup>
            {spaces}
          </ListGroup>
        </ListGroupItem>
